app.controller('groupManagerCtrl', function($scope, req, $state,
		myPaginationService) {
	var vm = this;
	...
	vm.getAllGroup = function() {
		req.post("groupManager/list", {
			rows : $scope.groupPageObject.pageSize,
			page : $scope.groupPageObject.currentPage,
		}).then(
				function successCallback(response, status, headers) {
					...
					$scope.groupPageObject.totalPage = totalRecord % maxResult == 0 ? (totalRecord / maxResult) : Math.ceil(totalRecord / maxResult) ;
					vm.list = response.data.data;
				})
	};
	$scope.$watch('groupPageObject.totalPage', function() {
		myPaginationService.showFirstPageContent($scope.groupPageObject, 1);
	});
